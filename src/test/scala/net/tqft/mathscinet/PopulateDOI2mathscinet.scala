...
object PopulateDOI2mathscinet extends App {
  ...
  for (group <- Articles.fromBibtexFile(System.getProperty("user.home") + "/projects/arxiv-toolkit/100_4.bib").grouped(1000); article <- { val p = group.par; p.tasksupport = pool; p }; doi <- article.DOI) {
    println(doi -> article.identifierString)
    bucket.update(doi, article.identifierString)
  }
}