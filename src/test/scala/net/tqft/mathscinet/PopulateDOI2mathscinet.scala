...
object PopulateDOI2mathscinet extends App {
  ...
  for (group <- Articles.fromBibtexFile(System.getProperty("user.home") + "/projects/arxiv-toolkit/100.bib").grouped(1000); article <- { val p = group.par; p.tasksupport = pool; p }; doi <- article.DOI) {
